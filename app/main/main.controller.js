...
export class MainController {
  constructor($log, $timeout, $scope, $q, toastr, cmMatrixViewFactory, cmModelFactory, cmMatrixFactory, cmGraphFactory,
    ...
    if (this.database == "marclab") {
      ...
      if (useLargeResult) {
        this.requestInitialData(self.resource + "/assets/mock/defaultMarclab.json");
      } else {
  ...
  activateWithClientOnlyData() {
    let self = this;
    let defaultDataFiles = [self.resource + 'assets/mock/2018.01.23-network-514-1-hop.json', self.resource + "assets/mock/2018.01.23-network-all-hops.json"];
    let defaultDataNames = ['small example', 'large example'];
    ...
    let requests = [];
    for (let i = 0; i < defaultDataFiles.length; ++i) {
      requests.push(self.$http.get(defaultDataFiles[i]));
    }
